...
# dirNames = ['sierra_2000','sierra_1000','sierra_0500','sierra_0250','sierra_0100']
# spds = [.02,.05,.16,.25,.40]
...
for k in range(len(name)):
    ...
    for l in range(len(dirNames)):
        ...
        if(l != 0 ):
            plt.plot(x[0,:],np.sum(plotData[0:zSlice,1:-1,:] * hFacC[0:zSlice,1:-1,:], axis=(0,1)),linewidth=2,label=dirName,color=colors[:,l-1])
            # plt.plot(x[0,:],np.sum(plotData[0:zSlice,1:-1,:], axis=(0,1)),linewidth=2,label=dirName,color=colors[:,l-1],linestyle='--')
        else:
            plt.plot(x[0,:],np.sum(plotData[0:zSlice,1:-1,:] * hFacC[0:zSlice,1:-1,:], axis=(0,1)),linewidth=2,label=dirName,color='xkcd:light gray')
            # plt.plot(x[0,:],np.sum(plotData[0:zSlice,1:-1,:], axis=(0,1)),linewidth=2,label=dirName,color='xkcd:light gray',linestyle='--')
        ax = plt.gca()
    ...
    plt.legend()  
    str = "figs/Flux%s%05i.png" % (name[k],j)
    plt.show()
