           ...
           elseif bergMaskNums(i,j) >= 1000 && bergMaskNums(i,j) < 10000
            ...
            % used?
             while exit_while == 0 && used_all_bergs == 0
                ...
                if abs(new_remainder) < abs(remainder) || exit_while == 1
                    % append new berg
                    local_berg_areas = [local_berg_areas(:);a_use(idx2)];
                    local_berg_widths = [local_berg_widths(:);w_use(idx2)];
                    local_berg_lengths = [local_berg_lengths(:);l_use(idx2)];
...
figure; 
subplot(1,3,1); imagesc(bergConc); colorbar; caxis([0 100]); title('target cover');
subplot(1,3,2); imagesc(derived_cover); colorbar; caxis([0 100]);  title('derived cover');
% ax3= subplot(1,3,3); imagesc(difference); colorbar; caxis([-5 5]); colormap(ax3, bluewhitered); title('difference');
ax3= subplot(1,3,3); imagesc(difference); colorbar; caxis([-5 5]); colormap(ax3, cmocean('balance')); title('difference');
