...
labels = ['Block + Drag', 'No Block + No Drag']
...
x_z = np.array(dataset['Z'])
x_u = np.array(dataset['U'])
x_v = np.array(dataset['V'])
x_w = np.array(dataset['W'])
x_spd = (x_u**2 + x_v**2 + x_w**2)**(0.5)
del x_u,x_v,x_w
...
x_zDeep = 38
zDeep = 27
x_xSlice = np.argmin(np.abs(x_x))
x_spd_melt = x_spd.copy()
x_spd_melt[x_fw == 0] = np.nan
plt.hist(x_spd[4,:x_zDeep,:,1:x_xSlice].flatten(),bins=50,label='Hughes',alpha=0.5,color="xkcd:gray")
plt.hist(x_spd_melt[4,:x_zDeep,:,1:x_xSlice].flatten(),bins=50,label='Hughes (Melt Only)',alpha=0.5,color="xkcd:dark gray")
...
for j in range(len(folders)):
    ...
    spd = (data[2,:zDeep,:,:xSlice]**2 + data[3,:zDeep,:,:xSlice]**2 + data[4,:zDeep,:,:xSlice]**2)**(0.5)
    
    if(j == 0):
        plt.hist(spd[:,:,:].flatten(),bins=50,label=labels[j],alpha=0.5,zorder=5,color=colors[j])
    else:
        plt.hist(spd[:,:,:].flatten(),bins=50,label=labels[j],alpha=0.5,color=colors[j])
str = "figs/spdHist%s.png" %fileName
plt.yscale('log')
...
x_spd_melt = x_spd.copy()
x_spd_melt[x_fw == 0] = np.nan
plt.plot(np.nanmean(x_spd[4,:x_zDeep,:,1:x_xSlice],axis=(1,2)),x_z[:x_zDeep],alpha=0.5,label='Hughes',color="xkcd:gray")
plt.plot(np.nanmean(x_spd_melt[4,:x_zDeep,:,1:x_xSlice],axis=(1,2)),x_z[:x_zDeep],alpha=0.5,label='Hughes (Melt Only)',color="xkcd:dark gray")
...
for j in range(len(folders)):
    ...
    if(j == 0):
        plt.plot(np.nanmean(spd[:,:,:],axis=(1,2)),z[:zDeep],label=labels[j],color=colors[j])
    else:
        plt.plot(np.nanmean(spd[:,:,:],axis=(1,2)),z[:zDeep],label=labels[j],color=colors[j])
str = "figs/spdDepth%s.png" %fileName
plt.grid(alpha=.5)
plt.ylabel('Depth [m]')
plt.xlabel('Speed [m/s]')
plt.legend()
