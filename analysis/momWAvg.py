...
dynName = ['momWDiag', 'momWDiag', 'momWDiag', 'momWDiag', 'momWDiag', 'dynDiag']
name = ['WSidDrag','Wm_Diss','Wm_Advec','momKE','PHI_NH','PHIHYD']
cbarLabel = ["[m/s^2]", "[m/s^2]", "[m/s^2]", "[m^2/s^2]","[m^2/s^2]","[m^2/s^2]"]
...
for k in kList:
    ...
    for i in np.arange(startStep, maxStep + 1, sizeStep):
        ...
        kk = k
        if k == 0:
            lvl = np.linspace(-1e-10,1e-10,31)
            cm = 'cmo.balance'
        elif k == 1:
            lvl = np.linspace(-1e-6,1e-6,31)
            cm = 'cmo.balance'
        elif k == 2:
            lvl = np.linspace(-1e-6,1e-6,31)
            cm = 'cmo.balance'
        elif k == 3:
            lvl = np.linspace(0,2e-2,31)
            cm = 'cmo.speed'
        elif k == 4:
            lvl = np.linspace(-1e-3,1e-3,31)
            cm = 'cmo.balance'
        elif k == 5:
            lvl = np.linspace(-200,200,31)
            cm = 'cmo.balance'
