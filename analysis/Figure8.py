...
for i in range(len(folders)):
    ...
    meltBelowSpd[spd > .04] = np.nan
    meltAboveSpd = dataMeltOnly.copy()
    meltAboveSpd[spd < .04] = np.nan
    ...
    ax2.plot(np.nansum(meltBelowSpd[:,:,:],axis=(1,2))/dz,z,label='spd < $U_{min}$',color='xkcd:light blue')
    ax2.plot(np.nansum(meltAboveSpd[:,:,:],axis=(1,2))/dz,z,label='spd > $U_{min}$',color='xkcd:dark blue')
# Wrap up after plotting everything
...
for j in range(len(folders)):
    ...
    x = mds.rdmds("%s%s/XC" %(folder,resultFolder))-melangeToe
