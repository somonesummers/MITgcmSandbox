...
for dynName in dynNameList:
    ...
    for k in kList:
        ...
        for i in np.arange(startStep, maxStep + 1, sizeStep):
            ...
            if(showDensity):
                salt = np.squeeze(np.average(dataQuiv[1,:,1:-1,:],weights=openFrac[:,1:-1,:],axis=1))
                ...
                CT = gsw.CT_from_t(salt, np.average(dataQuiv[0,:,1:-1,:],weights=openFrac[:,1:-1,:],axis=1), pressure)
