...
for i in range(len(args.files)):
    ...
    if(args.rampVariable[0] == 0):
        ...
        forceLabel = 'Melt Rate\n[m/day]'
        forceString = 'Melt'
        notforceTime = UcTime
        notforceLabel = 'Calving Rate\n[m/yr]'
        notforceString = 'Calving'
    elif(args.rampVariable[0] == 1):
        forceTime = UcTime
        forceLabel = 'Calving Rate\n[m/yr]'
        forceString = 'Calving'
        notforceTime = BTimeSmooth
        notforceLabel = 'Melt Rate\n[m/day]'
        notforceString = 'Melt'
    else:
...
plt.savefig('%sprettyRamp%sResults%s%s.png' %(dirStr,forceString,xString,multiStr),format='png',dpi=args.dpi)
if(args.silent == 0):
    plt.show()
plt.close()
