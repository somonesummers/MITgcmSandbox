...
for j in range(len(folders)):
    ...
    dynName = ['BRGFlx','dynDiag','ptraceDiag','plumeDiag','pickup','pickup_ptracers']
    ...
    for k in range(len(dynName)):
        ...
        for i in range(len(timeSteps)):
            if(os.path.isfile('results/%s.%010i.001.001.meta'%(dynName[k],timeSteps[i]))):
                data = mds.rdmds("%s%s/%s"%(folder,resultFolder, dynName[k]), int(timeSteps[i]))
                mds.wrmds('results/%s' %dynName[k],data,itr=int(timeSteps[i]), dataprec='float32')
