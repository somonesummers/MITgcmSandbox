...
years = args.Years
for k in np.linspace(0,364,72,dtype=int):#np.linspace(0,364,365,dtype=int):
    ...
    for i in range(len(years)):
        ...
        for j in toIterate:
            ...
            bFluxTime[j] = -1*simpson(data.B, x=data.X_)  #m^2/y
        ...
        ax4.plot(np.linspace(0,1,20),BShapeTime[DOY_to_plot,:],alpha = 1,linestyle=lStyle[i],color=colorList[int(DOY_to_plot / 73)],linewidth=lWidth)
    ...
    plt.savefig(f'figs/PrettyPlotMovie_{k+1:05d}.png',format='png',dpi=args.dpi)
    if(args.silent == 0):
        plt.show()
    plt.close()

os.system(f'magick -delay 20 figs/PrettyPlotMovie_*.png -colors 256 -depth 256 figs/PPMovie{years[0]}.gif')
os.system(f'ffmpeg -f image2 -r 10 -i figs/PrettyPlotMovie_%05d.png -c:v h264_videotoolbox -crf 22 figs/PPMovie_yr{years[0]}.mp4')

#Clean up intermediate pngs
if(True):
    os.system('rm -f figs/PrettyPlotMovie_*.png')
