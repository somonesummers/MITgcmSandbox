...
def createSBATCHfile_Sherlock(run_config, cluster_params, walltime_hrs, email, 
    ...
    loadList = cluster_params['load_list']
...
def getPickupIter(exp_path, pickup_yr=-1, max_yr_diff=1, results_path=''):
    
    """
    Helper function to retrieve pickup iter for a given year
    
    exp_path: path to experiment directory
    pickup_yr: year of pickup file. If -1, final year is used
    max_yr_diff: maximum number of years pickup_yr needs to be from closest pickup year (default: 1 year)
    results_path: path to results directory. If specified, this overrides params['results_path']
    ...
    # extract iters
    iters_strs = [f.split('.')[1] for f in pickup_files]
    iters = np.array(iters_strs).astype(int)
    
    if pickup_yr==-1:
        last_iter_yr = iters[-1]*params['deltaT']/secsInYr
        print("Last iter=%s; last iter year=%.2f" %(iters[-1], last_iter_yr))
        ...
        idx = np.argmin(np.abs(pickup_yrs-pickup_yr))
        time_diff = np.abs(pickup_yrs[idx]-pickup_yr)
        
        err_msg = "No pickup files found within %s year of year: %s. Closest is year %s."%(max_yr_diff,
                                                                                          pickup_yr, 
                                                                                          pickup_yrs[idx])
    
        assert time_diff <max_yr_diff, err_msg
    
        print("Closest pickup year=%s, iter_str=%s" %(pickup_yrs[idx], iters_strs[idx]))

        return iters_strs[idx]
    
    
def createNewBranch(exp_path_old, exp_path_new, branch_year=-1, overwrite=False):
    
    """
    Function to create a new experiment directory that branches from an existing one.
    
    Input:
    exp_path_old: path to old experiment directory
    exp_path_new: path of new experiment directory
    branch_year: year from which to branch (corresponding pickup file must exist in exp_path_old)
    overwrite: If True, script deletes exp_name_new if it exists. 
    
    """
    
    # create new_exp_name if necessary
    params,_ = raf.get_exp_params_py(exp_path_old)
    exp_dir_root = "/".join(exp_path_old.split('/')[:-1])
    exp_list = os.listdir(exp_dir_root)

    # create new exp directory directory
    #exp_path_new = os.path.join(exp_dir, exp_name_new)
    
    # check if exp_path_new directory exists 
    if os.path.exists(exp_path_new):
        if overwrite:
            response = input("WARNING %s already exists.\nContinue overwite [yes|no]"%exp_path_new)
            if response=='yes':
                
                # delete existing exp directory
                shutil.rmtree(exp_path_new)
                
                # create new directory
                os.makedirs(exp_path_new, exist_ok=True)
                os.makedirs(os.path.join(exp_path_new, 'results/'), exist_ok=True)
            else:
                print("Exiting...")
                return 
        else:
            print("Error: %s already exists" %exp_path_new)
    ...
    print("copying/linking files to new exp directory. May take a few minutes...")  
    
    shutil.copytree(params['input_path'], os.path.join(exp_path_new, 'input/'))
    shutil.copytree(os.path.join(params['exp_path'], 'build/'),  os.path.join(exp_path_new, 'build/'))
    shutil.copytree(os.path.join(params['exp_path'], 'code/'),  os.path.join(exp_path_new, 'code/'))
    
    iter_str = getPickupIter(exp_path_old, branch_year)
        

    # copy over results data
    old_results_path = os.path.join(params['exp_path'], 'results')
    new_results_path = os.path.join(exp_path_new, 'results')
    ...
    # link datafiles associated with branch_year (no need to make copies of large files)
    ...
    source_fname = os.path.join(exp_path_new, 'source_info.txt')
